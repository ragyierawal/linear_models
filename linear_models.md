...
nyc_airbnb %>% 
  ...
  mutate(
    lm_fits = map(.x = data, ~lm(price ~ stars + room_type, data = .)),
...
Have to specify that this is a generalized model, and make sure to tell
what distribution this is coming from
